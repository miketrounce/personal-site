/* ═══════════════════════════════════════════════════════════════════
   CHART_RENDERERS — Chart.js rendering functions keyed by chart slug.
   Used by both the slides browse page and the deck presentation page.
   ═══════════════════════════════════════════════════════════════════ */

var SERIES_API = 'https://api.miketrounce.com/api/series/';
function commas(n) { return n.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ','); }

var CHART_RENDERERS = {

  'deficit-fan': function(canvas) {
  ...
  'debt-combo': function(canvas) {
    fetch(SERIES_API + '?name=' + encodeURIComponent('Gross loan debt of national government'))
      .then(function(r) { return r.json(); })
      .then(function(barData) {
        fetch(SERIES_API + '?name=' + encodeURIComponent('Gross debt-to-GDP ratio'))
          .then(function(r) { return r.json(); })
          .then(function(lineData) {
            ...
            var axisTopLabels = {
              ...
              afterDraw: function(chart) {
                var ctx = chart.ctx; var yLeft = chart.scales.y; var yRight = chart.scales.y1;
                ctx.save(); ctx.font = 'bold 11px -apple-system, system-ui, sans-serif'; ctx.textBaseline = 'bottom';
                ctx.fillStyle = '#81b64c'; ctx.textAlign = 'left'; ctx.fillText('ZAR bn', yLeft.left, yLeft.top - 8);
                ctx.fillStyle = '#e8c547'; ctx.textAlign = 'right'; ctx.fillText('Debt / GDP %', yRight.right, yRight.top - 8);
                ctx.restore();
              }
            };
            new Chart(canvas, {type:'bar', data:{labels:barData.labels, datasets:[
              ...
              layout:{padding:{top:20}}, interaction:{mode:'index', intersect:false},
  ...
  'deficit-combo': function(canvas) {
    Promise.all([
      fetch(SERIES_API + '?name=' + encodeURIComponent('Main budget deficit')).then(function(r){return r.json();}),
      fetch(SERIES_API + '?name=' + encodeURIComponent('Main budget balance')).then(function(r){return r.json();})
    ]).then(function(ds) {
  ...
  'debt-fan': function(canvas) {
    fetch(SERIES_API + '?name=' + encodeURIComponent('Gross debt-to-GDP ratio') + '&projections=' + encodeURIComponent('Gross debt-to-GDP ratio (projection)'))
  ...
  'foreign-line': function(canvas) {
    fetch(SERIES_API + '?name=' + encodeURIComponent('Foreign holdings of government bonds'))
      .then(function(r){return r.json();}).then(function(data) {
  ...
  'sabo-bar': function(canvas) {
    fetch(SERIES_API + '?name=' + encodeURIComponent('SA Net Bond Sales to Foreigners'))
      .then(function(r){return r.json();}).then(function(data) {
  ...
  'investor-composition': function(canvas) {
    var seriesNames = [
      'Bond holdings: Non-residents', 'Bond holdings: Banks', 'Bond holdings: Pension funds',
      'Bond holdings: Insurers', 'Bond holdings: Other financial', 'Bond holdings: Other'
    ];
    ...
    Promise.all(seriesNames.map(function(n) {
      return fetch(SERIES_API + '?name=' + encodeURIComponent(n)).then(function(r) { return r.json(); });
  ...
  'debt-service-bar': function(canvas) {
    fetch(SERIES_API + '?name=' + encodeURIComponent('Debt Service Cost % of Revenue'))
      .then(function(r){return r.json();}).then(function(data) {
  ...
  'yield-line': function(canvas) {
    fetch(SERIES_API + '?name=' + encodeURIComponent('SA 10Y Government Bond Yield'))
      .then(function(r){return r.json();}).then(function(data) {
  ...
  'real-yield-combo': function(canvas) {
    Promise.all([
      fetch(SERIES_API + '?name=' + encodeURIComponent('SA 10Y Government Bond Yield')).then(function(r){return r.json();}),
      fetch(SERIES_API + '?name=' + encodeURIComponent('CPI Inflation (monthly)')).then(function(r){return r.json();})
    ]).then(function(ds) {
      ...
      yieldData.labels.forEach(function(l,i) {
        var cpi = cpiMap[l];
        if (cpi != null) {
          labels.push(l); yieldVals.push(yieldData.values[i]); cpiVals.push(cpi);
  ...
  'yield-vs-foreign-combo': function(canvas) {
    Promise.all([
      fetch(SERIES_API + '?name=' + encodeURIComponent('Bond holdings: Non-residents')).then(function(r){return r.json();}),
      fetch(SERIES_API + '?name=' + encodeURIComponent('SA 10Y Government Bond Yield')).then(function(r){return r.json();})
    ]).then(function(ds) {
      ...
      foreignData.labels.forEach(function(l,i) {
        var y = yieldMap[l];
        if (y != null) { labels.push(l); foreignVals.push(foreignData.values[i]); yieldVals.push(y); }
      });
